...
(function initHeroVideos() {
  ...
  videos.forEach(function(video) {
    // Safari fix : forcer .play() dès que le navigateur est prêt
    // (autoplay HTML seul n'est pas fiable sur Safari sans geste utilisateur)
    function tryPlay() {
      var p = video.play();
      if (p !== undefined) p.catch(function() {});
    }

    // Tentative immédiate, puis sur les événements de chargement
    tryPlay();
    video.addEventListener('loadedmetadata', tryPlay, { once: true });
    video.addEventListener('canplay', tryPlay, { once: true });

    // Pause quand la vidéo sort du viewport, reprise à l'entrée
...
(function initAutoCapitalize() {
  ...
  ["prenom","nom","prenom2","nom2","lieu"].forEach(function(id) {
